...
{
    public sealed record RemoveCheckListItemCommand(Guid CheckListId, Guid ItemId) : ICommand<Unit>;
    ...
    internal sealed class RemoveCheckListItemCommandHandler(
        ICheckListRepository checkListRepository,
        ICurrentUser currentUser,
        IUnitOfWork unitOfWork) : ICommandHandler<RemoveCheckListItemCommand, Unit>
    {
        public async Task<Result<Unit>> Handle(RemoveCheckListItemCommand command, CancellationToken cancellationToken)
        {
            ...
            if (!checkList.RemoveItem(command.ItemId))
