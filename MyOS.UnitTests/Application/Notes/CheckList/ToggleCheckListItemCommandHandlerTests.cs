...
namespace MyOS.UnitTests.Application.Notes.CheckList
{
    public class ToggleCheckListItemCommandHandlerTests
    {
        ...
        private ToggleCheckListItemCommandHandler CreateHandler() =>
        ...
        public async Task Handle_ListDoesNotExist_ReturnsNotFoundAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new ToggleCheckListItemCommand(Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None);
        ...
        public async Task Handle_ListOwnedByAnotherUser_ReturnsForbiddenAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new ToggleCheckListItemCommand(othersList.Id, item.Id), CancellationToken.None);
        ...
        public async Task Handle_ItemNotInList_ReturnsItemNotFoundAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new ToggleCheckListItemCommand(list.Id, Guid.NewGuid()), CancellationToken.None);
        ...
        [Fact]
        public async Task Handle_OwnedListWithItem_TogglesItemAndSavesOnce()
        {
            var list = DomainCheckList.Create(_currentUser.Id, "L");
            ...
            var result = await CreateHandler().Handle(
                new ToggleCheckListItemCommand(list.Id, item.Id), CancellationToken.None);
