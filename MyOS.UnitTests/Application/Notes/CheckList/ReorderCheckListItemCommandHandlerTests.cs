...
namespace MyOS.UnitTests.Application.Notes.CheckList
{
    public class ReorderCheckListItemCommandHandlerTests
    {
        ...
        private ReorderCheckListItemCommandHandler CreateHandler() =>
        ...
        public async Task Handle_ListDoesNotExist_ReturnsNotFoundAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new ReorderCheckListItemCommand(Guid.NewGuid(), Guid.NewGuid(), 1), CancellationToken.None);
        ...
        public async Task Handle_ListOwnedByAnotherUser_ReturnsForbiddenAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new ReorderCheckListItemCommand(othersList.Id, item.Id, 1), CancellationToken.None);
        ...
        public async Task Handle_ItemNotInList_ReturnsItemNotFoundAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new ReorderCheckListItemCommand(list.Id, Guid.NewGuid(), 1), CancellationToken.None);
        ...
        [Fact]
        public async Task Handle_OwnedListWithItem_ReordersAndSavesOnce()
        {
            var list = DomainCheckList.Create(_currentUser.Id, "L");
            var first = list.AddItem("a");  // order 1
            var second = list.AddItem("b"); // order 2
            _checkLists.GetByIdAsync(list.Id, Arg.Any<CancellationToken>()).Returns(list);

            var result = await CreateHandler().Handle(
                new ReorderCheckListItemCommand(list.Id, second.Id, 1), CancellationToken.None);

            result.IsSuccess.ShouldBeTrue();
            second.Order.ShouldBe(1);
            first.Order.ShouldBe(2);
