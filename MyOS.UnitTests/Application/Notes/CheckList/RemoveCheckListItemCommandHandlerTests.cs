...
namespace MyOS.UnitTests.Application.Notes.CheckList
{
    public class RemoveCheckListItemCommandHandlerTests
    {
        ...
        private RemoveCheckListItemCommandHandler CreateHandler() =>
        ...
        public async Task Handle_ListDoesNotExist_ReturnsNotFoundAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new RemoveCheckListItemCommand(Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None);
        ...
        public async Task Handle_ListOwnedByAnotherUser_ReturnsForbiddenAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new RemoveCheckListItemCommand(othersList.Id, item.Id), CancellationToken.None);
        ...
        public async Task Handle_ItemNotInList_ReturnsItemNotFoundAndDoesNotSave()
        {
            ...
            var result = await CreateHandler().Handle(
                new RemoveCheckListItemCommand(list.Id, Guid.NewGuid()), CancellationToken.None);
        ...
        [Fact]
        public async Task Handle_OwnedListWithItem_SoftDeletesItemAndSavesOnce()
        {
            var list = DomainCheckList.Create(_currentUser.Id, "L");
            ...
            var result = await CreateHandler().Handle(
                new RemoveCheckListItemCommand(list.Id, item.Id), CancellationToken.None);

            result.IsSuccess.ShouldBeTrue();
            item.DeletedAtUtc.ShouldNotBeNull();
