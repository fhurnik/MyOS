...
{
    public class UpdateCardioDurationCommandHandlerTests
    {
        private readonly IWorkoutRepository _workouts = Substitute.For<IWorkoutRepository>();
        private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();
        private readonly FakeCurrentUser _currentUser = new();

        private UpdateCardioDurationCommandHandler CreateHandler() =>
        ...
        public async Task Handle_WorkoutDoesNotExist_ReturnsNotFound()
        {
            ...
            var result = await CreateHandler().Handle(
                new UpdateCardioDurationCommand(Guid.NewGuid(), Guid.NewGuid(), 900), CancellationToken.None);
        ...
        public async Task Handle_WorkoutOwnedByAnotherUser_ReturnsForbidden()
        {
            ...
            var result = await CreateHandler().Handle(
                new UpdateCardioDurationCommand(othersWorkout.Id, Guid.NewGuid(), 900), CancellationToken.None);
        ...
        public async Task Handle_WorkoutExerciseNotInWorkout_ReturnsWorkoutExerciseNotFound()
        {
            ...
            workout.AddCardioExercise(Guid.NewGuid(), duration: 600);
            _workouts.GetByIdAsync(workout.Id, Arg.Any<CancellationToken>()).Returns(workout);

            var result = await CreateHandler().Handle(
                new UpdateCardioDurationCommand(workout.Id, Guid.NewGuid(), 900), CancellationToken.None);

            result.Error.ShouldBe(WorkoutErrors.WorkoutExerciseNotFound);
        }

        [Fact]
        public async Task Handle_EntryIsStrengthNotCardio_ReturnsActivityTypeMismatch()
        {
            var workout = OwnedWorkout();
            var strengthEntry = workout.AddStrengthExercise(Guid.NewGuid()); // no duration
            _workouts.GetByIdAsync(workout.Id, Arg.Any<CancellationToken>()).Returns(workout);

            var result = await CreateHandler().Handle(
                new UpdateCardioDurationCommand(workout.Id, strengthEntry.Id, 900), CancellationToken.None);
        ...
        [Fact]
        public async Task Handle_CardioEntry_ChangesDurationAndSavesOnce()
        {
            var workout = OwnedWorkout();
            var cardioEntry = workout.AddCardioExercise(Guid.NewGuid(), duration: 600);
            _workouts.GetByIdAsync(workout.Id, Arg.Any<CancellationToken>()).Returns(workout);

            var result = await CreateHandler().Handle(
                new UpdateCardioDurationCommand(workout.Id, cardioEntry.Id, 900), CancellationToken.None);

            result.IsSuccess.ShouldBeTrue();
            cardioEntry.Duration.ShouldBe(900);
