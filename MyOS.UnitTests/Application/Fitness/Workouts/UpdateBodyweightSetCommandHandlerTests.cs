...
{
    public class UpdateBodyweightSetCommandHandlerTests
    {
        ...
        private UpdateBodyweightSetCommandHandler CreateHandler() =>
            new(_workouts, _exercises, _currentUser, _unitOfWork);

        private (Workout workout, WorkoutExercise entry, ExerciseSet set) OwnedWorkoutWithBodyweightSet()
        {
            var workout = Workout.Create(_currentUser.Id, new DateOnly(2026, 6, 21), null);
            var entry = workout.AddStrengthExercise(Guid.NewGuid());
            var set = entry.AddBodyweightSet(10, addedWeight: null, negatives: null, rir: 2);
            return (workout, entry, set);
        }

        private static UpdateBodyweightSetCommand Command(Guid workoutId, Guid entryId, Guid setId) =>
            new(workoutId, entryId, setId, Reps: 12, AddedWeight: 5m, Negatives: 2, Rir: 1);
        ...
        {
            var (workout, _, set) = OwnedWorkoutWithBodyweightSet();
        ...
        {
            var (workout, entry, _) = OwnedWorkoutWithBodyweightSet();
            _workouts.GetByIdAsync(workout.Id, Arg.Any<CancellationToken>()).Returns(workout);
        ...
        {
            var (workout, entry, set) = OwnedWorkoutWithBodyweightSet();
            _workouts.GetByIdAsync(workout.Id, Arg.Any<CancellationToken>()).Returns(workout);
        ...
        [Fact]
        public async Task Handle_ValidBodyweightSet_UpdatesSetAndSavesOnce()
        {
            var (workout, entry, set) = OwnedWorkoutWithBodyweightSet();
            _workouts.GetByIdAsync(workout.Id, Arg.Any<CancellationToken>()).Returns(workout);
            _exercises.GetByIdAsync(entry.ExerciseId, Arg.Any<CancellationToken>())
